-- =============================================================================
-- 0024_search_drivers_vehicle_strict.sql
--
-- ช่องติ๊ก "เฉพาะคนที่เคยวิ่งประเภทรถนี้" (p_vehicle_type_strict) ให้ประเภทรถสลับได้
-- เหมือนเส้นทางใน 0023 แต่ค่าเริ่มต้นตรงข้ามกัน: ประเภทรถ default = true (กรองเข้มเหมือน
-- 0019/0023 ทุกประการ) เพราะเป็นพฤติกรรมที่ใช้งานอยู่แล้ว ถ้าตั้งเริ่มต้นเป็น false การค้นด้วย
-- ประเภทรถอย่างเดียวจะกลายเป็นได้ทุกคนทันที ส่วนเส้นทางเริ่มต้นเป็น false ตามเหตุผลของ 0019
-- (ช่องพิมพ์อิสระ สะกดไม่ตรงแล้วคนหาย)
--
-- ปิดติ๊ก (false) = ประเภทรถเป็นแค่ตัวจัดลำดับ คนที่เคยวิ่งประเภทนั้นขึ้นก่อน ไม่ตัดใครออก
-- (ลำดับความเกี่ยวข้องชั้นที่ 3 ใช้ vehicle_type_jobs อยู่แล้ว)
--
-- ต้อง drop ลายเซ็นของ 0023 ก่อน ด้วยเหตุผลเดียวกับที่ 0023 drop ของ 0019
-- =============================================================================

drop function if exists search_drivers(text, text, text, boolean, int);
...
where d.status in ('active', 'probation')   -- ชั้นที่ 1: กรองแข็ง — ตรงกับ StatusDialog.tsx
  -- ลูกค้า: กรองแข็งเสมอ (รหัสมาตรฐาน) — ไม่ระบุก็ผ่าน
  and (p_customer_code is null or coalesce(r.customer_jobs, 0) > 0)
  -- ประเภทรถ: กรองแข็งเมื่อ strict (ค่าเริ่มต้น) — coalesce(…, true) กัน null หลุดเข้ามาแล้ว
